...
public class Damage : MonoBehaviour {
	...
	{
		if (damageOnTrigger) {
			if (this.tag == "Chicken" && collision.gameObject.tag == "Sword")	
				return;
	...
	{	
		if (damageOnCollision) {
			if (this.tag == "Sword" && collision.gameObject.tag == "Chicken")	// if the player got hit with it's own bullets, ignore it
				return;
